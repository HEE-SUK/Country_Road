...
public class EnemyMovement : MonoBehaviour
{
    ...
    private float stopPos = 0f;
    public void Init(float stopPos, float enemySpeed, float carSpeed, CallBack tryAttack)
    { // 좀비 테이블 속도 전달 // ex) 0.2f
    ...
    void Update()
    {
        ...
        if (transform.position.z > -8 && isAttackChance)
        { // 공격범위 일때 
            tryAttack();
            isAttackChance = false;
            return;
        }
        if (transform.position.z < stopPos && curSpeed > 0) // 한계점 까지 밀렸을때
            return;
